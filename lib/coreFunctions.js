(function(exports) {
    function isEqualTo(test, compare) {
        ...
        var message =  "Expected " + test + " to equal " + compare;
        return new Result(result, message);
    }
    function isGreaterThan(test, compare) {
        var result = test > compare ? true : false;
        var message =  "Expected " + test + " to be greater than " + compare;
        return new Result(result, message);
    }
    function isLessThan(test, compare) {
        var result = test < compare ? true : false;
        var message =  "Expected " + test + " to be less than " + compare;
        return new Result(result, message);
    }
    function isOdd(test) {
        var result = test % 2 != 0 ? true : false;
        var message =  "Expected " + test + " to be odd "
        return new Result(result, message);
    }
    function isEven(test) {
        var result = test % 2 == 0 ? true : false;
        var message =  "Expected " + test + " to be even "
        return new Result(result, message);
    }
    function isEmpty(test) {
        var result = test.length == 0 ? true : false;
        var message =  "Expected " + test + " to be empty "
        return new Result(result, message);
    }
    function isInteger(test) {
        var result = parseInt(parseFloat(test)) === test ? true : false;
        var message =  "Expected " + test + " to be an Integer "
        return new Result(result, message);
    }
    function isNumber(test) {
        var result = parseInt(parseFloat(test)) == test ? true : false;
        var message =  "Expected " + test + " to be a number "
        return new Result(result, message);
    }
    ...
    function isNull(test) {
        var result = test === null ? true : false;
        var message =  "Expected " + test + " to be null "
        return new Result(result, message);
    }
